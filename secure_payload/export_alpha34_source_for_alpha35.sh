...
BUILD_SCRIPT=/tmp/build_alpha34_export.sh
...
for file in "$BUILD_SCRIPT" "$PATCH30" "$PATCH31" "$PATCH32" "$PATCH33" "$PATCH34"; do
  test -s "$file"
done
...
path = Path('/tmp/build_alpha34_export.sh')
...
source = source.replace('0.2.1-alpha.30', '0.2.1-alpha.34')
source = source.replace('run_alpha30_stable', 'run_alpha34_stable')
source = source.replace('build_alpha30_overlay', 'build_alpha34_overlay')

chmod_anchor = 'chmod +x /tmp/run_alpha34_stable.sh\n'
if chmod_anchor not in source:
    raise SystemExit('run chmod anchor missing')
inject = r'''python3 - <<'PYEXPORT'
from pathlib import Path
p = Path('/tmp/run_alpha34_stable.sh')
...
export_code = [
    ...
    "    'rm -rf /tmp/exported_alpha34\\n'\n",
    "    'mkdir -p /tmp/exported_alpha34\\n'\n",
    "    'cp -R \\\"$SOURCE/payload/lib\\\" /tmp/exported_alpha34/lib\\n'\n",
    "    'cp -R \\\"$SOURCE/payload/test\\\" /tmp/exported_alpha34/test\\n'\n",
    "    'cp \\\"$SOURCE/payload/app_icon.png\\\" /tmp/exported_alpha34/app_icon.png\\n'\n",
    "    'exit 0\\n\\n'\n",
    "    '# Restore the encrypted foreground monitor and boot receiver.\\n',\n",
    "    1,\n",
    ")\n",
...
run_line = 'bash /tmp/run_alpha34_stable.sh\n'
...
test -s /tmp/exported_alpha34/lib/wizard_v4.dart
test -s /tmp/exported_alpha34/lib/services.dart
test -s /tmp/exported_alpha34/lib/official_seat_web.dart
test -s /tmp/exported_alpha34/lib/cgv_seat_bridge.dart
test -s /tmp/exported_alpha34/lib/background_monitor.dart
grep -q 'CGV 1회 연결 및 좌석 가져오기' /tmp/exported_alpha34/lib/wizard_v4.dart
grep -q '__cineseatCaptureInstalled' /tmp/exported_alpha34/lib/cgv_seat_bridge.dart
