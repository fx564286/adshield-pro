...
APK="$DIST/cineseat-0.2.1-alpha.36-VALIDATION.apk"
...
tail = tail.replace('bash /tmp/run_alpha30_stable.sh\n', '', 1)
tail = tail.replace('0.2.1-alpha.30', '0.2.1-alpha.36')
tail = tail.replace('alpha30-fixed.apk', 'alpha36-fixed.apk')
tail = tail.replace("versionCode='50'", "versionCode='56'")
tail = tail.replace(
    '- CGV and Megabox direct APIs and hidden official non-member WebViews start automatically in parallel',
    '- CGV seats use direct searchIfSeatDataK/searchIfSeatData HTTP paths without automatic homepage fallback',
)
...
RUNTIME="$ROOT/secure_runtime"
'''
Path('/tmp/sign_alpha36_fixed.sh').write_text(header + tail, encoding='utf-8')
PY
chmod +x /tmp/sign_alpha36_fixed.sh
...
- CGV seat failure no longer starts the official homepage or login WebView automatically
...
- exact alpha35 fixed signing identity is retained for overwrite installation
REPORT

rm -f "$RUNTIME/native.aes.key" "$RUNTIME/native.tar.gz"
