...
BUILD_SCRIPT=/tmp/build_alpha34_base.sh
...
path = Path('/tmp/build_alpha34_base.sh')
...
source = source.replace(
    ...
    + "    'grep -q \"cgvSeatSessionBootstrapJavaScript\" \"$SOURCE/payload/lib/cgv_seat_bridge.dart\"\\n' +\n"
    + "    'grep -q \"__cineseatCaptureInstalled\" \"$SOURCE/payload/lib/cgv_seat_bridge.dart\"\\n' +\n"
    + "    'grep -q \"XMLHttpRequest.prototype.open\" \"$SOURCE/payload/lib/cgv_seat_bridge.dart\"\\n' +\n"
    + "    'grep -q \"CGV 1회 연결 및 좌석 가져오기\" \"$SOURCE/payload/lib/wizard_v4.dart\"\\n' +\n"
    + "    'grep -q \"minWidth: 390\" \"$SOURCE/payload/lib/official_seat_web.dart\"\\n' +\n"
    + "    'grep -q \"Duration(seconds: 5)\" \"$SOURCE/payload/lib/services.dart\"\\n' +\n",
    1,
)

source = source.replace('0.2.1-alpha.30', '0.2.1-alpha.34')
source = source.replace('version: 0.2.1-alpha.34+50', 'version: 0.2.1-alpha.34+54')
source = source.replace('--build-number=50', '--build-number=54')
source = source.replace("versionCode='50'", "versionCode='54'")
source = source.replace('alpha30-fixed.apk', 'alpha34-fixed.apk')
...
source = source.replace('alpha30 version marker', 'alpha34 version marker')
source = source.replace(
    'fixed alpha27 signing certificate retained for overwrite installation over alpha29',
    'fixed signing certificate retained for overwrite installation over alpha33',
)
